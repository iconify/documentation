...
function wrap(type: string, text: string): string {
	return (
		(wrappers[type] === void 0 ? '' : wrappers[type].header) +
		text +
		(wrappers[type] === void 0 ? '' : wrappers[type].footer)
	);
}
	...
	return (
		...
		types
			.map((type) => {
				...
				if (midSeparators.indexOf(type) !== -1) {
					return type;
				}
				return (
 ...
 * Inline code
 */
export function renderInlineCode(context: MDContext, md: md) {
	md.renderer.rules.code_inline = function (tokens, idx, options, env, slf) {
		...
		if (rawContent.slice(0, 1) !== '[') {
			throw new Error(
				`Inline code token is missing type in ${context.filename}: ${token.content}`
			);
		}
		...
		if (index === -1) {
			throw new Error(
				`Bad inline code token in ${context.filename}: ${token.content}`
			);
		}
		let type = rawContent.slice(0, index);
		...
		switch (type) {
			...
			case 'value':
				return (() => {
					let valueType = 'string';
					if (escapedContent === 'true' || escapedContent === 'false') {
						valueType = 'literal';
					} else if (escapedContent.match(/^-?[0-9.]+$/)) {
						valueType = 'number';
					}

					return wrap(
						'value',
						'<span class="hljs-' + valueType + '">' + escapedContent + '</span>'
					);
				})();
			*/
			...
			default:
				throw new Error(
					`Invalid inline code token type in ${context.filename}: ${type}`
				);
		}
	};
}
